---
title: "Preprocessing network Bern"
...
network <- full_network %E>% # use full network here!
  filter(is.na(access) | access %in% c("yes","customers","destination","permissive","unknown","public","residents","24/7","residential","official") | bicycle %in% c("yes","designated","permissive","private","destination","unknown")) %N>% filter(!node_is_isolated())
...
network <- network %E>%
  filter(highway %in% relevant_highway | bicycle %in% c("yes","designated","permissive","private","destination","unknown") | footway %in% c("crossing","crossway","link","traffic_island")) %N>% filter(!node_is_isolated())
...
```{r}
network <- network %E>%
  filter(!(vehicle %in% c("agricultural","forestry","no","permit","delivery","agricultural;forestry","delivery;forestry")) | is.na(vehicle) | bicycle %in% c("yes","designated","permissive","private","destination","unknown")) %N>% filter(!node_is_isolated())
...
network <- addedge(914, 917, "track")
network <- addedge(4083, 4068, "footway")
network <- addedge(6793, 6886, "track")
network <- addedge(8673, 8765, "footway")
network <- addedge(8672, 8614, "footway")
network <- addedge(6962, 8607, "footway")
network <- addedge(8755, 13866, "footway")
network <- addedge(13542, 9647, "footway")
network <- addedge(13562, 9830, "footway")
network <- addedge(9594, 9341, "footway")
network <- addedge(10947, 10961, "living_street")
network <- addedge(310, 4760, "track")
network <- addedge(4762, 4774, "track")
network <- addedge(14257, 14270, "footway")
network <- addedge(12345, 12394, "living_street")
network <- addedge(5010, 4950, "footway")
network <- addedge(11060, 10999, "footway")
network <- addedge(10998, 10997, "footway")
...
## Check distance of population points to nearest network node
```{r}
...
# Distances of population points to nearest network node
...
```{r}
# Save preprocessed network
saveRDS(network, file = here::here("data", "bern/network_be.rds"))
edges <- network %E>% as_tibble() %>% st_as_sf()
nodes <- network %N>% as_tibble() %>% st_as_sf()

st_write(edges, dsn = here::here("data", "bern/edges_be.shp"), delete_layer = TRUE)
st_write(nodes, dsn = here::here("data", "bern/nodes_be.shp"), delete_layer = TRUE)
```
