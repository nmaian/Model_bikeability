...
network <- readRDS(here::here("data", "bern/network_quality.rds"))
...
distances_dest_be <- st_distance(destinations, nearest_point_dest, by_element = TRUE)
distances_pop_be <- st_distance(statpop, nearest_point_pop, by_element = TRUE)

hist(distances_dest_be, xlab = "Distance [m]", freq = TRUE, main = "Distance into network for destination points")
hist(distances_pop_be, xlab = "Distance [m]", freq = TRUE, main = "Distance into network for population points")
```
...
write_csv(statpop_df, file = file.path(dataFolder, "bern/statpop_be_bikeability.csv"))
```
