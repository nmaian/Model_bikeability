...
network <- readRDS(here::here("data", "zurich/network_quality.rds"))
...
distances_dest_zh <- st_distance(destinations, nearest_point_dest, by_element = TRUE)
distances_pop_zh <- st_distance(statpop, nearest_point_pop, by_element = TRUE)

hist(distances_dest_zh, xlab = "Distance [m]", freq = TRUE, main = "Distance into network for destination points")
hist(distances_pop_zh, xlab = "Distance [m]", freq = TRUE, main = "Distance into network for population points")
```
...
calcAccessibility_weighted <- function(desttype, dmax, beta, cost_string){
  ...
  for(i in 1:nrow(cost_mat)){
    dists_within_dmax <- cost_mat[i, cost_mat_real[i, ] <= dmax] # select those distances below dmax of real distance
...
write_csv(statpop_df, file = file.path(dataFolder, "zurich/statpop_zh_bikeability.csv"))
```
