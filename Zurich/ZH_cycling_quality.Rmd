...
# network <- readRDS(here::here("data", "zurich/network_zh.rds"))
edges <- st_read(file.path(dataFolder, "zurich/edges_zh.shp"))
...
links_dem <- read.csv(file.path(dataFolder, "links_dem_zh.csv"), sep = ",", header = FALSE)
...
saveRDS(dem, file = here::here("data", "dem2m_zh.rds"))

dem <- readRDS(here::here("data", "dem2m_zh.rds"))

# Load swissSURFACE dataset
links_surface <- read.csv(file.path(dataFolder, "zurich/swisssurface_zh.csv"), sep = ",", header = FALSE)
...
filenames <- list.files(file.path(dataFolder, "surface_zh"))
swisssurface <- readLAS(file.path(dataFolder, paste0("surface_zh/", filenames[1])), select = c("xyzc"), filter = "-keep_class 17")@data # only take points from bridges
unlink(temp)
...
for(i in 2:length(filenames)){
  ...
  filename <- unzip(temp, list = TRUE)[1, 1]
  unzip(temp)
  part_surf <- readLAS(file.path(dataFolder, paste0("surface_zh/", filenames[i])), select = c("xyzc"), filter = "-keep_class 17")@data # only take points from bridges
  unlink(temp)
...
# Save surface data
saveRDS(swisssurface, file = here::here("data", "zurich/surface_zh.rds"))

swisssurface <- readRDS(here::here("data", "zurich/surface_zh.rds"))
...
# Trees swissTLM3D
trees <- st_as_sf(st_read(file.path(dataFolder, "trees/swissTLM3D_TLM_EINZELBAUM_GEBUESCH_OST.shp")))
...
```{r}
# Only keep accidents, which are at most 10m away from an edge of the bike network: Of the 2334 accidents 2257 are kept
...
write_csv(edges_acc_df, file.path(dataFolder, "zurich/edges_accidents.csv"))

# Load accident data
edges_accidents <- read.csv(file.path(dataFolder, "zurich/edges_accidents.csv"), sep = ",")
network <- network %E>%
...
st_write(edges_dem, dsn = here::here("data", "zurich/edges_dem.shp"), delete_layer = TRUE)
...
network <- network %E>%
  dplyr::mutate(slope = ifelse(from == 21309 & to == 23281, 0, slope),
                slope = ifelse(from == 21310 & to == 23274, 0, slope),
                slope = ifelse(from == 23266 & to == 23281, 0, slope),
                slope = ifelse(from == 21379 & to == 23267, 0.02, slope),
                slope = ifelse(from == 21309 & to == 23281, 0, slope),
                slope = ifelse(from == 20475 & to == 23270, 0, slope),
                slope = ifelse(from == 21311 & to == 23271, 0, slope),
                slope = ifelse(from == 13010 & to == 13328, 0, slope),
                slope = ifelse(from == 4573 & to == 4574, 0.03, slope),
                slope = ifelse(from == 373 & to == 374, 0, slope),
                slope = ifelse(from == 375 & to == 377, 0, slope),
                slope = ifelse(from == 9985 & to == 9986, 0, slope))
...
write_csv(edges_slope_df, file.path(dataFolder, "zurich/edges_slope.csv"))
st_write(edges_slope, dsn = here::here("data", "zurich/edges_slope.shp"), delete_layer = TRUE)

# Load slope data
edges_slope <- read.csv(file.path(dataFolder, "zurich/edges_slope.csv"), sep = ",")
network <- network %E>%
...
- Unclassified: Mostly minor roads => impute mean value of 30 for NA values
...
# network <- network %E>%
#   dplyr::filter(highway != "service" | (highway == "service" & (btw_centrality > 77481 | bicycle != "unknown" | !(is.na(maxspeed))))) %N>%
...
# network <- network %E>%
#   dplyr::filter(highway != "path" | (highway == "path" & (btw_centrality > 25829 | bicycle != "unknown" | !(is.na(maxspeed))))) %N>% 
#   dplyr::filter(!node_is_isolated())
# 
# network_df <- network %E>% data.frame()
# highway_sum <- aggregate(network_df$weight, by = list(network_df$highway), FUN = sum) %>% dplyr::arrange(desc(x))
...
```{r}
# Filter trees near to segments of the network: Of the 115421 trees, 48803 are still included
...
st_write(greenaquatic_areas, dsn = here::here("data", "zurich/greenaquatic_areas/greenaquatic_areas_zh.shp"), delete_layer = TRUE)
greenaquatic_areas <- st_as_sf(st_read(file.path(dataFolder, "zurich/greenaquatic_areas/greenaquatic_areas_zh.shp")))
```
...
# Load data from ssh
edges_greenaquatic <- read.csv(file.path(dataFolder, "zurich/edges_greenaquatic.csv"), sep = ",")
network <- network %E>%
...
st_write(edges_quality, dsn = here::here("data", "zurich/edges_quality.shp"), delete_layer = TRUE)
saveRDS(network, file = here::here("data", "zurich/network_quality.rds"))
```
