...
# network <- readRDS(here::here("data", "lausanne/network_ls.rds"))
edges <- st_read(file.path(dataFolder, "lausanne/edges_ls.shp"))
...
links_dem <- read.csv(file.path(dataFolder, "links_dem_ls.csv"), sep = ",", header = FALSE)
...
saveRDS(dem, file = here::here("data", "dem2m_ls.rds"))

dem <- readRDS(here::here("data", "dem2m_ls.rds"))

# Load swissSURFACE dataset
links_surface <- read.csv(file.path(dataFolder, "lausanne/swisssurface_ls.csv"), sep = ",", header = FALSE)
...
filenames <- list.files(file.path(dataFolder, "surface_ls"))
swisssurface <- readLAS(file.path(dataFolder, paste0("surface_ls/", filenames[1])), select = c("xyzc"), filter = "-keep_class 17")@data # only take points from bridges
unlink(temp)
...
for(i in 2:length(filenames)){
  ...
  filename <- unzip(temp, list = TRUE)[1, 1]
  unzip(temp)
  part_surf <- readLAS(file.path(dataFolder, paste0("surface_ls/", filenames[i])), select = c("xyzc"), filter = "-keep_class 17")@data # only take points from bridges
  unlink(temp)
...
# Save surface data
saveRDS(swisssurface, file = here::here("data", "lausanne/surface_ls.rds"))

swisssurface <- readRDS(here::here("data", "lausanne/surface_ls.rds"))
...
```{r}
# Only keep accidents, which are at most 10m away from an edge of the bike network: Of the 527 accidents 517 are kept
...
Assessing the included and excluded accidents visually, a threshold of 10m has been found to fit well. Some accidents are excluded, since they lie on a way not included in the network, which makes sense. Some are excluded due to border effects. 10m fit well, because accidents happening on the side of a wide road, i.e. on a cycling lane, are still included if the street axis lies in the middle. And furthermore, accidents happening on a regular street are excluded if there is separate cycling infrastructure and regular street is excluded in network, since it has the tag use_sidepath. At complex locations with over- and underpassing streets like Escher-Wyss or Hardbrücke 1-3 points probably get assigned to wrong segment, but overall the matching works also well for these locations, assuming that the location of the accident is correct. 
...
write_csv(edges_acc_df, file.path(dataFolder, "lausanne/edges_accidents.csv"))

# Load accident data
edges_accidents <- read.csv(file.path(dataFolder, "lausanne/edges_accidents.csv"), sep = ",")
network <- network %E>%
...
st_write(edges_dem, dsn = here::here("data", "lausanne/edges_dem.shp"), delete_layer = TRUE)
# detach("package:plyr", unload = TRUE) # unload plyr package for convenience
```

### Swisssurface3D: Surface bridges
```{r}
...
nearest_point <- find_nearest_node(nodes_bridges, surface_bridges)
summary(st_distance(nodes_bridges, nearest_point, by_element = TRUE))
quantile(st_distance(nodes_bridges, nearest_point, by_element = TRUE), 0.90)
```
...
```{r}
# Calculate slope for bridges separately using bridges points only
# network <- network %E>% 
#   dplyr::mutate(id = as.numeric(c(1:n())))
segments_bridges <- bridges %E>% st_as_sf() 
#   %>% dplyr::mutate(id = as.numeric(c(1:n())))
segments_points_bridges <- st_cast(segments_bridges, "POINT") # get points of all segments
segments_points_bridges$seq = ave(segments_points_bridges$id, segments_points_bridges$id, FUN = seq_along) # counts sequence for points per edge
flag = ddply(
  st_set_geometry(segments_points_bridges, NULL),
  "id",
  summarize,
  flag = seq == min(seq) | seq == max(seq)
  )$flag # values TRUE or FALSE for each point: TRUE for point at start or end of a segment
segments_points_bridges = segments_points_bridges[flag, ] # filter first and last (= TRUE / flags from all points)
type_flag = ddply(
  st_set_geometry(segments_points_bridges, NULL),
  "id",
  summarize,
...
segments_points_bridges$elevationtype = type_flag$elevationtype

first = segments_points_bridges[segments_points_bridges$elevationtype == "first", ]
last = segments_points_bridges[segments_points_bridges$elevationtype == "last", ]

# Extract data
dat = data.frame(ID = first$id, elev1 = NA, elev2 = NA, dist = first$weight, stringsAsFactors = FALSE)

get_elev_bridges <- function(points, nodes){
  points %>%
  mutate(
    index_of_nearest_feature = st_nearest_feature(., nodes),
    elev_near = nodes[index_of_nearest_feature, "Z"] %>% pull("Z"),
    dist_nearest = st_distance(., nodes[index_of_nearest_feature, ], by_element = TRUE)
    )
}

first <- get_elev_bridges(first, surface_bridges)
last <- get_elev_bridges(last, surface_bridges)

# Only assign elevation if nearest elevation point is <30m away
first <- first %>% 
  dplyr::mutate(elev_near = ifelse(drop_units(first$dist_nearest) > 30, NA, elev_near))
last <- last %>% 
  dplyr::mutate(elev_near = ifelse(drop_units(last$dist_nearest) > 30, NA, elev_near))

dat$elev1 = first$elev_near
dat$elev2 = last$elev_near
...
dat_slopes <- dat[ ,-c(2, 3, 4)] %>% drop_na()
slopes <- dat_slopes

# Replace slope, if slope using bridges points could be calculated
net_df <- network %E>% as_tibble(spatial = FALSE)
net_bridges_slope <- left_join(net_df, dat_slopes, by = c("id" = "ID"))

network <- network %E>%
  dplyr::mutate(
    slope = ifelse(!(is.na(net_bridges_slope$slope)), net_bridges_slope$slope, slope_dem)
  )
detach("package:plyr", unload = TRUE) # unload plyr package for convenience
```
...
# Set slopes of bridges with slope > 12% (unrealistic) to slope using DEM
network <- network %E>%
  dplyr::mutate(slope = ifelse(slope > 0.12 & (bridge == "yes" | bridge == "viaduct" | bridge == "covered"), slope_dem, slope)
                )
```
...
network <- network %E>%
  dplyr::mutate(slope = ifelse(from == 6583 & to == 7775, 0, slope),
                slope = ifelse(from == 9944 & to == 9946, 0, slope)
                )
...
write_csv(edges_slope_df, file.path(dataFolder, "lausanne/edges_slope.csv"))
st_write(edges_slope, dsn = here::here("data", "lausanne/edges_slope.shp"), delete_layer = TRUE)

# Load slope data
edges_slope <- read.csv(file.path(dataFolder, "lausanne/edges_slope.csv"), sep = ",")
network <- network %E>%
...
- Secondary: NA streets are mostly 50 streets and 50 is also mean value => impute mean value of 50 for NA values
...
- Tertiary link: Streets which have a value mostly have 60 and ones with NA can mostly be classified as 50 => impute mean value of 50 for NA values
...
- Primary: Streets with NA values are mostly faster streets and they should be rather large streets => impute rounded mean value of 60 for NA values
- Unknown: Missing values are mostly segments for motorized private traffic and mostly 30 as speed limit => impute 30 for NA values
```{r}
# Define speed limit for different types of highways with NA values
type_high20 <- c("living_street")
type_high30 <- c("residential","unclassified","unknown")
type_high50 <- c("secondary","tertiary","tertiary_link","primary_link","secondary_link")
type_high60 <- c("primary")
...
network <- network %E>%
  dplyr::mutate(maxspeed = ifelse(is.na(maxspeed) & (highway %in% type_high20), 20, 
                       ...
                       ifelse(is.na(maxspeed) & (highway %in% type_high50), 50, 
                              ifelse(is.na(maxspeed) & (highway %in% type_high60), 60, maxspeed)))))
```
...
# network <- network %E>%
#   dplyr::filter(highway != "service" | (highway == "service" & (btw_centrality > 71877 | bicycle != "unknown" | !(is.na(maxspeed))))) %N>%
...
# network <- network %E>%
#   dplyr::filter(highway != "path" | (highway == "path" & (btw_centrality > 23961 | bicycle != "unknown" | !(is.na(maxspeed))))) %N>% 
#   dplyr::filter(!node_is_isolated())
# 
# network_df <- network %E>% data.frame()
# highway_sum <- aggregate(network_df$weight, by = list(network_df$highway), FUN = sum) %>% dplyr::arrange(desc(x))
...
```{r}
# Filter trees near to segments of the network: Of the 81576 trees, 34266 are still included
...
st_write(greenaquatic_areas, dsn = here::here("data", "lausanne/greenaquatic_areas/greenaquatic_areas_ls.shp"), delete_layer = TRUE)
greenaquatic_areas <- st_as_sf(st_read(file.path(dataFolder, "lausanne/greenaquatic_areas/greenaquatic_areas_ls.shp")))
```
...
# Load data from ssh
edges_greenaquatic <- read.csv(file.path(dataFolder, "lausanne/edges_greenaquatic.csv"), sep = ",")
network <- network %E>%
...
st_write(edges_quality, dsn = here::here("data", "lausanne/edges_quality.shp"), delete_layer = TRUE)
saveRDS(network, file = here::here("data", "lausanne/network_quality.rds"))
```
