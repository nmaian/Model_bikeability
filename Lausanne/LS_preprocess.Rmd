---
title: "Preprocessing network Lausanne"
...
# City borders: Take bounding box of community boundaries, since community consist of multiple parts
...
network <- addedge(8176, 8174, "cycleway")
network <- addedge(13927, 11044, "cycleway")
network <- addedge(6654, 6655, "path")
network <- addedge(7039, 7040, "footway")
network <- addedge(7275, 6528, "footway")
network <- addedge(7395, 7390, "pedestrian")
network <- addedge(5539, 5536, "footway")
network <- addedge(6661, 6664, "footway")
network <- addedge(17825, 17831, "path")
network <- addedge(17844, 17831, "path")
...
```{r}
# Split segments with a length >400m into 6 parts for later slope calculation
...
shortened_edges <- line_segment(long_edges, n_segments = 6)
...
```{r}
# Save preprocessed network
saveRDS(network, file = here::here("data", "lausanne/network_ls.rds"))
edges <- network %E>% as_tibble() %>% st_as_sf()
nodes <- network %N>% as_tibble() %>% st_as_sf()

st_write(edges, dsn = here::here("data", "lausanne/edges_ls.shp"), delete_layer = TRUE)
st_write(nodes, dsn = here::here("data", "lausanne/nodes_ls.shp"), delete_layer = TRUE)
```
