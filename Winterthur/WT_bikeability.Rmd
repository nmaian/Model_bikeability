...
hospitals <- hospitals %>% st_filter(border) %>% filter(drop_units(weight) > 4000) %>% st_centroid() # keep only hospitals with area > 5000 m2
...
parks <- parks %>% st_filter(border) %>% filter(drop_units(weight) > 2000) %>% st_centroid() # keep only parks > 2500 m2
...
distances_dest_wt <- st_distance(destinations, nearest_point_dest, by_element = TRUE)
distances_pop_wt <- st_distance(statpop, nearest_point_pop, by_element = TRUE)

hist(distances_dest_wt, xlab = "Distance [m]", freq = TRUE, main = "Distance into network for destination points")
hist(distances_pop_wt, xlab = "Distance [m]", freq = TRUE, main = "Distance into network for population points")
```
...
write_csv(statpop_df, file = file.path(dataFolder, "winterthur/statpop_wt_bikeability.csv"))
```
