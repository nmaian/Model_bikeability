---
title: "Preprocessing network Winterthur"
...
Vehicle = private included since after inspected results visually it makes sense: Segments are still allowed to be used by bike and mostly last connectetion to buildings (no transit)
```{r}
network <- network %E>%
...
tm_shape(st_as_sf(main_graph %>% activate("nodes"), "nodes")) +
  tm_dots()
# tm_shape(st_as_sf(full_network %>% activate("edges") %>% filter(footway == "crossing" | highway == "crossing")), "edges") +
#   tm_lines(col = "red")
```
...
network <- addedge(8948, 8762, "cycleway")
network <- addedge(8950, 8810, "path")
network <- addedge(8818, 8811, "cycleway")
network <- addedge(4208, 3075, "track")
network <- addedge(4216, 4219, "path")
network <- addedge(6605, 6591, "cycleway")
network <- addedge(191, 415, "path")
network <- addedge(169, 188, "path")
network <- addedge(7290, 73, "cycleway")
network <- addedge(7237, 3644, "footway")
network <- addedge(7245, 3644, "footway")
network <- addedge(5941, 5078, "footway")
network <- addedge(10589, 5369, "footway")
...
```{r}
# Save preprocessed network
saveRDS(network, file = here::here("data", "winterthur/network_wt.rds"))
edges <- network %E>% as_tibble() %>% st_as_sf()
nodes <- network %N>% as_tibble() %>% st_as_sf()

st_write(edges, dsn = here::here("data", "winterthur/edges_wt.shp"), delete_layer = TRUE)
st_write(nodes, dsn = here::here("data", "winterthur/nodes_wt.shp"), delete_layer = TRUE)
```
