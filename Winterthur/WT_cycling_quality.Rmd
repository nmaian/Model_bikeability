...
# network <- readRDS(here::here("data", "winterthur/network_wt.rds"))
edges <- st_read(file.path(dataFolder, "winterthur/edges_wt.shp"))
...
links_dem <- read.csv(file.path(dataFolder, "links_dem_wt.csv"), sep = ",", header = FALSE)
...
saveRDS(dem, file = here::here("data", "dem2m_wt.rds"))

dem <- readRDS(here::here("data", "dem2m_wt.rds"))
...
```{r}
# Only keep accidents, which are at most 10m away from an edge of the bike network: Of the 595 accidents 587 are kept
...
write_csv(edges_acc_df, file.path(dataFolder, "winterthur/edges_accidents.csv"))

# Load accident data
edges_accidents <- read.csv(file.path(dataFolder, "winterthur/edges_accidents.csv"), sep = ",")
network <- network %E>%
...
st_write(edges_dem, dsn = here::here("data", "winterthur/edges_dem.shp"), delete_layer = TRUE)
...
network <- network %E>%
  dplyr::mutate(slope = ifelse(from == 4377 & to == 4380, 0, slope),
                slope = ifelse(from == 3959 & to == 4095, 0, slope),
                slope = ifelse(from == 317 & to == 319, 0, slope)
                )
...
write_csv(edges_slope_df, file.path(dataFolder, "winterthur/edges_slope.csv"))
st_write(edges_slope, dsn = here::here("data", "winterthur/edges_slope.shp"), delete_layer = TRUE)

# Load slope data
edges_slope <- read.csv(file.path(dataFolder, "winterthur/edges_slope.csv"), sep = ",")
network <- network %E>%
...
```{r}
# Assign "path" to bridleway, which is explicitly allowed for bikes
network <- network %E>%
  dplyr::mutate(highway = ifelse(highway == "bridleway", "path", highway))
```
...
# network <- network %E>%
#   dplyr::filter(highway != "service" | (highway == "service" & (btw_centrality > 25798 | bicycle != "unknown" | !(is.na(maxspeed))))) %N>%
...
# network <- network %E>%
#   dplyr::filter(highway != "path" | (highway == "path" & (btw_centrality > 10462 | bicycle != "unknown" | !(is.na(maxspeed))))) %N>% 
#   dplyr::filter(!node_is_isolated())
# 
# network_df <- network %E>% data.frame()
# highway_sum <- aggregate(network_df$weight, by = list(network_df$highway), FUN = sum) %>% dplyr::arrange(desc(x))
...
```{r}
# Filter trees near to segments of the network
...
st_write(greenaquatic_areas, dsn = here::here("data", "winterthur/greenaquatic_areas/greenaquatic_areas_wt.shp"), delete_layer = TRUE)
greenaquatic_areas <- st_as_sf(st_read(file.path(dataFolder, "winterthur/greenaquatic_areas/greenaquatic_areas_wt.shp")))
```
...
# Load data from ssh
edges_greenaquatic <- read.csv(file.path(dataFolder, "winterthur/edges_greenaquatic.csv"), sep = ",")
network <- network %E>%
...
st_write(edges_quality, dsn = here::here("data", "winterthur/edges_quality.shp"), delete_layer = TRUE)
saveRDS(network, file = here::here("data", "winterthur/network_quality.rds"))
```
